...
def draw_galaxy(
    galaxy: Galaxy, ax: Axes = plt.subplots(figsize=(5, 2.7), layout="constrained")
):
    ...
    for planet in galaxy.planets:
        circ = plt.Circle(
            ...
            planet.radius,
            color=planet.owner.colour if planet.owner else "grey",
        )
        ax.add_patch(circ)
    ...
    for troops in galaxy.planets + galaxy.fleets:
        ...
        if troops.owner:
            if not troops.owner.id in total_troops:
                total_troops[troops.owner.id] = {
                    "colour": troops.owner.colour,
                    "troop_count": 0,
                }
    ...
    for i, player in enumerate(total_troops):
        ax.text(
            (i - 1) * 1 / len(total_troops),
            0.01,
            total_troops[player]["troop_count"],
            verticalalignment="bottom",
            horizontalalignment="right",
            # transform=ax.transAxes,
            color=total_troops[player]["colour"],
            fontsize=12,
            fontweight="bold",
        )

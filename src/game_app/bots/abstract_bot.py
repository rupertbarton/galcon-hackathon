...
class AbstractBot():
    ...
    def own_planets(self):
        return [planet for planet in self.current_state.planets if self.is_own_planet(planet)]

    @property
    def enemy_planets(self):
        return [planet for planet in self.current_state.planets if self.is_enemy_planet(planet)]
    
    @property
    def ally_planets(self):
        return [planet for planet in self.current_state.planets if self.is_friendly_planet(planet) and not self.is_own_planet(planet)]

    @property
    def neutral_planets(self):
        return [planet for planet in self.current_state.planets if self.is_neutral_planet(planet)]

    @property
    def friendly_planets(self):
        return [planet for planet in self.current_state.planets if self.is_friendly_planet(planet)]
